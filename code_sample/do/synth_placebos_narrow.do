clear all
set more off
log using ../log/synth_placebos_narrow.log, replace

*************************************
**# Placebos for adults 1977 - 1989 *
*************************************
...
forvalues k = 1/51 {
	forvalues i = 1/3 {
		...
		forvalues z = 1/3 {
			...
			use ${dataset}, clear
			synth ${outcome} ${outcome}${xperiod} ${predictors}, trunit(`k') trperiod(1982) resultsperiod${rperiod} nested allopt keep(../int/adult_placebo_${outcome}_${group}_narrow_`k'.dta, replace) fig
			graph export ../figures/adult_placebo_${outcome}_${group}_narrow_`k'.png, replace
	}
	}
}

***************************************
**# Placebos for children 1977 - 1989 *
***************************************
...
forvalues k = 1/51 {
	forvalues i = 1/7 {
		...
		forvalues z = 1/3 {
			...
			use ${dataset}, clear
			synth ${outcome} ${outcome}${xperiod} ${predictors}, trunit(`k') trperiod(1982) resultsperiod${rperiod} nested allopt keep(../int/child_placebo_${outcome}_${group}_narrow_`k'.dta, replace) fig
			graph export ../figures/child_placebo_${outcome}_${group}_narrow_`k'.png, replace
	}
	}
}
